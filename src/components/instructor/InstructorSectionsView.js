...
import { years, semesters, GRADEBOOK_URL } from '../../Constants';
...
const InstructorSectionsView = () => {
  ...
  const [email, setEmail] = useState("dwisneski@csumb.edu");
  ...
  const handleSubmit = async () => {
    setSubmitted(true);
    fetch(`${GRADEBOOK_URL}/sections?email=${email}&year=${year}&semester=${semester}`)
          ...
          <TableBody>
            {sections.map((section) => (
              <TableRow key={section.secNo}>
                ...
                <TableCell sx={{ textTransform: 'capitalize' }}>{section.instructorName}</TableCell>
                <TableCell>
                  <Link to="/enrollments" state={section}>View Enrollments</Link>
                </TableCell>
                <TableCell>
